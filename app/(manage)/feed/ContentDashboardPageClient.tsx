...
export default function ContentDashboardPageClient() {
  ...
  const fetchPosts = React.useCallback(
    async (loadMore = false, search = '', accountId?: string) => {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {posts.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center text-muted-foreground">No posts found for your search term.</CardContent>
            </Card>
          ) : (
            posts.map((post) => (
